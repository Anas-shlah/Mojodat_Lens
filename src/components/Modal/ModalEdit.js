...
const ModalEdit = props => {
  ...
  const [valueBarcode, setValueBarcode] = useState({
    handling: true,
    value: modalEdit.Barcode,
  });
  const [valueName, setValueName] = useState({
    handling: true,
    value: modalEdit.AssetDescription,
  });
  let initialCategory = modalEdit.Category;
  let initialBarcode = modalEdit.Barcode;
  const {asset} = useSelector(state => state.assetReducer);
  const dispatch = useDispatch();

  useEffect(() => {
    setValueDropdown(modalEdit.Category);
    setValueBarcode({handling: true, value: modalEdit.Barcode});
    setValueName({handling: true, value: modalEdit.AssetDescription});
  }, [modalEdit]);

  const deletAsset = () => {
    try {
      const index = asset.findIndex(el => el.Category == initialCategory);
      let newAsset = [...asset];

      let filter = newAsset[index].data.filter(
        el => el.Barcode != initialBarcode,
      );
      newAsset[index].data = filter;
      ...
      <Modal
        ...
        <View style={styles.centeredView2}>
          <View style={styles.modalView}>
            <Text style={styles.title}>Edite Asset</Text>
            <CustomDropdown
              data={asset}
            ...
            <View style={[styles.horizontal, {justifyContent: 'space-around'}]}>
              ...
              <Pressable
                ...
                <Text style={[styles.textbuttonDelet, styles.textStyle]}>
                  delet
                </Text>
              </Pressable>
            </View>
